...
public class App 
{
    ...
    public static void main( String[] args ) throws IOException
    {
		...
		String[] arg = {"-n=dzzw", "-u=root", "-p=Yangkong@123a",
				"-d=C:\\Users\\User\\Desktop", "-t=mysql", "h=rm-bp1sy5io5e8e5g3y3zo.mysql.rds.aliyuncs.com", "p=3306"};
    	...
    	Map<String, String> map = Check(arg);
    ...
    public static void MySQL(Map<String,String> map) throws IOException{
    	//默认生成的文件名
    	String outFile = map.get("-d")+"/数据库表结构(dzzw).docx";
    	//查询表的名称以及一些表需要的信息
    ...
    private static void createDoc(ResultSet rs,String sqls,Map<String,String> map,String outFile,boolean type,String title,Connection con) throws IOException{
		...
		for(Map<String, String> str : list){
			RowRenderData header = getHeader(str.get("table_name")+"");
			log.info(str);
			i++;
			...
			data.put("name", new TextRenderData(str.get("table_name"), POITLStyle.getBtStyle()));
     ...
     */
    private static RowRenderData getHeader(String tableName){
    	/*RowRenderData header = RowRenderData.build(
				new TextRenderData("属性名（中文）", POITLStyle.getHeaderStyle()),
				new TextRenderData("属性", POITLStyle.getHeaderStyle()),
		...
		RowRenderData header = RowRenderData.build(
				new TextRenderData("表名："+tableName),
				new TextRenderData(""),
				new TextRenderData(""),
				new TextRenderData(""),
				new TextRenderData(""));
    ...
    private static List<RowRenderData> getRowRenderData(ResultSet set) {
    	...
    	try {
    		int i = 0;
			RowRenderData hrow = RowRenderData.build(
					new TextRenderData("属性名（中文）"),
					new TextRenderData("属性"),
					new TextRenderData("类型"),
					new TextRenderData("长度"),
					new TextRenderData("备注"));
			hrow.setStyle(POITLStyle.getHeaderTableStyle());
			result.add(hrow);
			while(set.next()){
				type = set.getString("data_type")+"";
				i++;
				RowRenderData row = RowRenderData.build(
						...
						new TextRenderData(type),
						new TextRenderData("int".equals(type)?"11":"tinyint".equals(type)?"4":set.getString("character_maximum_length")+"")
						);
				result.add(row);
